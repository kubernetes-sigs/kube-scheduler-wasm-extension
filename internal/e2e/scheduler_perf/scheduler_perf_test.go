...
import (
	...
	"sigs.k8s.io/kube-scheduler-wasm-extension/internal/e2e/scheduler_perf/extender"
	...
	defaultMetricsCollectorConfig = metricsCollectorConfig{
		Metrics: map[string]*labelValues{
			"scheduler_framework_extension_point_duration_seconds": {
				...
				values: []string{"PreFilter", "Filter", "PreScore", "Score", "PreBind", "Bind"},
			},
...
func getPodStrategy(cpo *createPodsOp) (testutils.TestPodCreateStrategy, error) {
	...
	if cpo.PodTemplatePath != nil {
		...
		// if basePod.Name != "" {
		// 	basePod.Name += strconv.Itoa(count)
		// 	count++
		// }
	}
