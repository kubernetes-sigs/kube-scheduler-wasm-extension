...
func TestEncodeClusterEvents(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "label",
			input: []api.ClusterEvent{{Resource: api.Node, ActionType: api.Add, Label: "NodeAdd"}},
			expected: []byte{
				byte(api.Node), 0, 0, 0,
				byte(api.Add), 0, 0, 0,
				'N', 'o', 'd', 'e', 'A', 'd', 'd', 0,
		...
		},
		{
			...
			input: []api.ClusterEvent{
				{Resource: api.Node, ActionType: api.UpdateNodeLabel | api.UpdateNodeCondition, Label: "NodeAdd"},
			},
			expected: []byte{
				byte(api.Node), 0, 0, 0,
		...
		},
		{
			name: "two",
			input: []api.ClusterEvent{
				{Resource: api.Node, ActionType: api.Add, Label: "NodeAdd"},
				{Resource: api.PersistentVolume, ActionType: api.Delete, Label: "PvDelete"},
			},
			expected: []byte{
				byte(api.Node), 0, 0, 0,
				byte(api.Add), 0, 0, 0,
		...
		{
			name:  "unicode",
			input: []api.ClusterEvent{{Resource: api.Node, ActionType: api.Add, Label: "fóo"}},
			expected: []byte{
				byte(api.Node), 0, 0, 0,
				byte(api.Add), 0, 0, 0,
				'f', 0xc3, 0xb3, 'o', 0,
