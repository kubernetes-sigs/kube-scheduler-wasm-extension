...
func encodeClusterEvents(input []api.ClusterEvent) []byte {
	...
	var size int
	for i := range input {
		size += minEncodedClusterEvent + len(input[i].Label)
	}

	// Write the NUL-terminated values to a byte slice.
	...
	for i := range input {
		...
		// Write the label NUL-terminated
		copy(encoded[pos+8:], e.Label)
		// No need to write zero to the end as make([]byte, size) zero fills.
		pos += 8 + len(e.Label) + 1 // +1 for NUL-terminator
	}
	return encoded
}
